...
const GROUPS: SeedGroup[] = [
  {
    ...
    tasks: [
      ...
      { id: "A.3", title: "Neon Postgres + DB push", description: "Schema materialized in Neon · 23 tables · 50+ indexes. Adapter via @prisma/adapter-neon. Validation: SELECT COUNT from information_schema.", effort: "S", status: "DONE" },
      { id: "A.4", title: "GitHub Actions CI · validate · build · test · ci-passed", description: "Required-status gate workflow at .github/workflows/ci.yml. Runs format-check + typecheck + lint + build (stub env vars) + test:run + ci-passed aggregator. Blocks merge if any step fails.", effort: "S", status: "DONE" },
      { id: "A.5", title: "Auto-merge workflow + autonomous-ready label", description: ".github/workflows/auto-merge.yml fires on autonomous-ready label + branch starting auto/ or enhance/. Runs gh pr merge --squash --auto --delete-branch. Labels: autonomous, autonomous-ready, needs-review created via API.", effort: "S", status: "DONE" },
      { id: "A.6", title: "Vercel deploy · mapsly.ai · dev.mapsly.ai", description: "Production domain wired to www.mapsly.ai. Subdomain dev.mapsly.ai via DNS CNAME → Vercel. Middleware host-based rewrite to /dev. Env vars in Vercel project. Postinstall runs prisma generate (INC-06).", effort: "S", status: "DONE" },
      { id: "A.7", title: "Incident-prevention system", description: ".claude/memory/incidents.md seeded with 15 INC entries. .claude/rules/incident-prevention.md as load-bearing rule. autonomous-build-loop SKILL mandates incidents read first + new entries before close.", effort: "M", status: "DONE" },
      { id: "A.8", title: "Native macOS launchd loop", description: "scripts/launchd/ — loop-tick.sh + ai.mapsly.loop.plist + install.sh + loop-prompt.md. Escapes Cowork FUSE limitation (INC-14). Runs every 5 min via launchd. Honors loop-lock.json state (idle/running/cooldown/paused).", effort: "M", status: "DONE" },
      {
        id: "A.9",
  ...
  },
  {
    ...
    tasks: [
      ...
      },
      {
        ...
        description: "On every new review, generate aiReplyDraftEn + aiReplyDraftEs. Tone matches SMB's reply-tone setting (warm/professional/casual). Stored on Review row. Files: modules/scoring/ai-reply.ts. Validation: unit + cohort A/B test eventually.",
        effort: "M",
        deps: "C.7",
      },
    ],
  },
  {
    ...
    tasks: [
      ...
      { id: "E.1", title: "SMB dashboard · /(smb)/dashboard", description: "Maria's home. Hero KPI: Mapsly Score 6.2/10. 5 supporting KPIs. 'What needs your attention today' (max 4 alert cards). Top 3 fixes ordered by impact. This-week activity feed. Files: app/[locale]/(smb)/dashboard/page.tsx. Validation: browser anon (redirect) + signed-in SMB + Lighthouse + ux-reviewer-smb + copy-reviewer (no jargon).", effort: "L", deps: "D.2,E.0,C.9" },
      { id: "E.2", title: "SMB reviews page · /(smb)/reviews", description: "Tabs: Unanswered (default) / Negative / All / By theme. Per review: stars + date + text + AI reply draft (EN+ES toggle). 'Post to Google' CTA. Right rail: rating distribution, theme analysis, reply-tone settings. Files: app/[locale]/(smb)/reviews/page.tsx. Validation: browser SMB + DB (AI reply persistence) + ux + copy.", effort: "L", deps: "D.6,D.7,E.0" },
      { id: "E.3", title: "SMB competitors page · /(smb)/competitors", description: "Head-to-head vs 3 nearest competitors. Service-coverage matrix. Threat ranking. 'Where you beat them' / 'Where they beat you' wedges. Files: app/[locale]/(smb)/competitors/page.tsx. Validation: browser + ux.", effort: "L", deps: "C.9,E.0" },
      { id: "E.4", title: "SMB search visibility · /(smb)/search", description: "Keyword table: term, monthly volume, your position, who else ranks in local 3-pack. P0 opportunities (high volume + low your-rank). Plain English: 'When someone searches X you appear Y'. Files: app/[locale]/(smb)/search/page.tsx. Validation: browser + ux + copy.", effort: "M", deps: "D.1,C.9,E.0" },
      { id: "E.5", title: "SMB ads page · /(smb)/ads", description: "Competitor ad copy by keyword (Meta + Google Ads Transparency). 'Off-keyword warning' when ads don't match SMB services. 14-keyword lane grid. Files: app/[locale]/(smb)/ads/page.tsx. Validation: browser + DB.", effort: "M", deps: "C.4,C.8,E.0" },
      { id: "E.6", title: "SMB settings · /(smb)/settings", description: "Business info (name, address, hours, services). Reply-tone (warm/professional/casual). Locale preference. Notification prefs. Stripe billing card. Files: app/[locale]/(smb)/settings/page.tsx. Validation: browser + DB persistence + form validation.", effort: "M", deps: "B.6,E.0,G.4" },
      { id: "E.7", title: "SMB onboarding flow", description: "4-step wizard: claim business → set vocabulary (industry) → connect Google Business Profile → invite team. Each step has skip. Mobile-friendly. Files: app/[locale]/(smb)/onboarding/page.tsx. Validation: browser multi-step + DB state per step + a11y.", effort: "L", deps: "B.6,E.0" },
    ],
  },
  {
    id: "F",
  ...
  },
  {
    ...
    sortOrder: 7,
    tasks: [
      { id: "H.1", title: "dev.mapsly.ai dashboard · core", description: "Hero tiles + cards (Loop control, DORA, Plan progress, Sessions 7d, Service health, Cron+API health, Auto-enhance signals, Recent commits, Open PRs). Refresh button + 30s AutoRefresh. Shipped.", effort: "XL", deps: "A.6", status: "DONE" },
      { id: "H.2", title: "Task tracker · DB-backed with TaskGroup + TaskRun models", description: "Postgres Task + TaskGroup + TaskRun + AgentInvocation + Notification + CostBudget. Seed from this file. Dashboard reads from DB. /dev/tasks list grouped by domain. Shipped v0.3.0+.", effort: "L", deps: "A.2", status: "DONE" },
      { id: "H.3", title: "/dev/tasks/[id] detail page with edit + run history + span tree", description: "Per-task page with editable form. Run history with 5-dim scorecard, agents used, gates passed, validation strategy + outcomes, AgentInvocation span tree (Honeycomb-style). Shipped v0.3.0+.", effort: "L", deps: "H.2", status: "DONE" },
      { id: "H.4", title: "Edit + Add + Delete tasks from UI", description: "Server actions backed by Prisma + Zod. Inline status menu on list. Full edit on detail. New-task modal per group. Soft-delete via SKIPPED status. Shipped v0.3.0.", effort: "M", deps: "H.3", status: "DONE" },
      { id: "H.5", title: "Loop reads next task from DB (not PLAN.md)", description: "Loop prompt updated to query Task table: status=PENDING, deps satisfied, priority+sortOrder ordering. On claim: status=IN_PROGRESS, open TaskRun. On close: write TaskRun + bump Task.status + denormalized scores. Shipped v0.3.0.", effort: "M", deps: "H.2", status: "DONE" },
      { id: "H.6", title: "process-enhancer agent · meta-loop (clusters incidents → opens self-improvement PRs)", description: "Daily scheduled. Reads incidents.md + last-N TaskRun + sessions JSON. Clusters patterns. Opens enhance/ PRs (rule rewrites, agent prompt tweaks). Writes enhance-signals.json. Dry-run script exists at scripts/process-enhancer-dryrun.ts. Validation: dry-run produces ≥1 signal on the 15 INC entries.", effort: "L", deps: "H.2", priority: 30 },
      ...
      { id: "H.8", title: "DORA metrics card + AgentInvocation span tree + Loop control card", description: "Derives deploy freq + lead time p50/p95 + change failure rate from existing TaskRun. AgentInvocation per-agent spans render as Honeycomb-style timeline on detail page. Loop control card with pause/resume/clear-cooldown. Shipped v0.3.3.", effort: "L", deps: "H.2", status: "DONE" },
      { id: "H.9", title: "Validation framework + Browser-testing rule", description: ".claude/rules/validation.md (per-task validation strategy: unit/integration/browser/DB/email/perf/a11y). .claude/rules/browser-testing.md (multi-user Claude in Chrome MCP). Test data lifecycle (seed→test→cleanup). Shipped v0.3.1.", effort: "M", deps: "H.5", status: "DONE" },
      { id: "H.10", title: "Notification + CostBudget models + post-merge rollback policy", description: "Notification table for loop→Viktor inbox (CRITICAL/ERROR/WARN/INFO). CostBudget per-scope daily/weekly/monthly enforcement. Post-merge Sentry health check + auto-revert if regression. Shipped v0.3.2.", effort: "M", deps: "H.2", status: "DONE" },
      { id: "H.11", title: "Agent orchestration rule · concurrency budget + sequencing", description: ".claude/rules/agent-orchestration.md. Caps research at 6 parallel, review at 5. Sequencing rules (code-reviewer before scorer, security before payments). Token-aware scheduling. Shipped v0.3.3.", effort: "S", deps: "H.5", status: "DONE" },
    ],
  },
  {
    id: "I",
    ...
    tasks: [
      ...
      { id: "I.2", title: "Spanish (es-US) translation of marketing copy", description: "messages/es.json populated for marketing namespace. Native-speaker review (no MT). Service vocabulary (patients → pacientes for med-spa).", effort: "M", deps: "I.1,B.1", tags: "human-required" },
      { id: "I.3", title: "Quebec French (fr-CA) translation", description: "messages/fr.json. Native Quebec French reviewer. CAD currency formatting. Locale-specific date formats.", effort: "M", deps: "I.1,B.1", tags: "human-required" },
      { id: "I.4", title: "Canadian English (en-CA) overrides", description: "messages/en-CA.json with only overrides vs en (cheque vs check, neighbour, etc.). Fallback chain to en. Files: messages/en-CA.json. Validation: routing-test for /en-ca path + locale-specific copy.", effort: "S", deps: "I.1", priority: 22 },
      { id: "I.5", title: "Currency + date locale-aware formatting", description: "useFormatter() throughout. en-US: $29.99 May 17. fr-CA: 29,99 $ CA 17 mai. Each Price + Date component reads locale from useLocale(). Files: components/i18n/Price.tsx, components/i18n/Date.tsx. Validation: snapshot tests per locale.", effort: "S", deps: "I.1", priority: 22 },
      { id: "I.6", title: "Translated route pathnames", description: "Per i18n/routing.ts pathnames: /for-agencies → /para-agencias / /pour-agences. /lists → /listas / /listes. Link from next-intl. Validation: routing test per locale + browser hreflang.", effort: "S", deps: "I.1,B.2" },
    ],
  },
];
