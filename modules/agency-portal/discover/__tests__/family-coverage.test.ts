// Pure unit tests for the family-coverage source of truth — the ONE derivation
// chain every surface shares (2026-07-06 truth unification):
// deriveTypeStates (9 types, the atom) → deriveGroupStates (7 data groups).
// The legacy 5-family model (deriveFamilyStates/FamilyState) was retired in
// Wave 2 — its honesty invariants now live on the per-type tests below.
...
describe("DATA_GROUPS + roll-up", () => {
  ...
  test("rollUpGroupState: enriched when ANY type has data (truth unification)", () => {
    ...
    // ANY type with data → enriched: the Boise ticket — AI research HAS data,
    // services sub-scan found no menu; the group must not hide the brief.
    const mixed = {
      ...allNotRun(),
      AI_RESEARCH: "enriched",
      SERVICES: "empty",
    };
    expect(rollUpGroupState(mixed as never, aiBrief)).toBe("enriched");
    // One enriched, one not_run → still enriched (data to show), never a
    // false "not_run" that would re-charge the ran type.
    const partial = { ...allNotRun(), CONTACTS: "enriched" };
    expect(rollUpGroupState(partial as never, contactsTech)).toBe("enriched");
    // Ran everywhere, nothing found → empty.
    const allEmpty = { ...allNotRun(), CONTACTS: "empty", TECH: "empty" };
  ...
  test("deriveGroupStates yields the full /7 denominator", () => {
    // Nothing ran → all 7 groups present, all not_run.
    const none = deriveGroupStates(allNotRun());
    expect(Object.keys(none).sort()).toEqual([...DATA_GROUP_KEYS].sort());
    expect(Object.values(none).every((s) => s === "not_run")).toBe(true);

    // Reviews + contacts+tech enriched → those 2 groups read enriched.
    ...
    expect(some.reviews).toBe("enriched");
    expect(some.contacts_tech).toBe("enriched");
    expect(some.site_speed).toBe("not_run");

    // A running type surfaces as a running group.
    const running = deriveGroupStates({ ...allNotRun(), SERP: "running" });
    expect(running.search).toBe("running");
  });
  ...
  test('"Enriched only" (per-lead) ignores per-cell ads/search scans', () => {
    ...
    expect(anyLeadGroupRan(cellOnly)).toBe(false); // no PERSONAL enrichment
