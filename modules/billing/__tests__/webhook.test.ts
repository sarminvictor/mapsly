...
function makePrisma(opts: { users?: FakeUser[]; agencies?: FakeAgency[] } = {}) {
    ...
    user: {
      findFirst: vi.fn(async ({ where }) => {
        for (const u of users.values()) {
          if (where.stripeSubscriptionId && u.stripeSubscriptionId === where.stripeSubscriptionId) {
            return { id: u.id, stripeSubscriptionId: u.stripeSubscriptionId };
          }
          if (where.stripeCustomerId && u.stripeCustomerId === where.stripeCustomerId) {
            return { id: u.id, stripeSubscriptionId: u.stripeSubscriptionId };
          }
        }
        return null;
      ...
      update: vi.fn(async ({ where, data }) => {
        userUpdates.push({ id: where.id, data: data as Record<string, unknown> });
    ...
    agency: {
      findFirst: vi.fn(async ({ where }) => {
        for (const a of agencies.values()) {
          if (where.stripeSubscriptionId && a.stripeSubscriptionId === where.stripeSubscriptionId) {
            return { id: a.id, stripeSubscriptionId: a.stripeSubscriptionId };
          }
          if (where.stripeCustomerId && a.stripeCustomerId === where.stripeCustomerId) {
            return { id: a.id, stripeSubscriptionId: a.stripeSubscriptionId };
          }
        }
        return null;
      ...
      update: vi.fn(async ({ where, data }) => {
        agencyUpdates.push({ id: where.id, data: data as Record<string, unknown> });
...
function checkoutCompleted(over: Partial<{ subscription: string; customer: string; metadata: Record<string, string>; mode: string }> = {}): Stripe.Event {
  return {
    id: "evt_co_1",
    ...
    data: {
      object: {
        ...
        metadata: over.metadata ?? { userId: "u1", plan: "smb_paid", audience: "smb" },
      } as unknown as Stripe.Checkout.Session,
    },
  } as Stripe.Event;
}

function subscriptionEvent(type: "customer.subscription.created" | "customer.subscription.updated" | "customer.subscription.deleted", over: Partial<{ id: string; customer: string; status: string; priceId: string; periodEnd: number; cancelAtPeriodEnd: boolean; metadata: Record<string, string> }> = {}): Stripe.Event {
  return {
    id: `evt_sub_${type}_1`,
    type,
    api_version: "2024-12-18.acacia",
...
function invoiceEvent(type: "invoice.paid" | "invoice.payment_failed", over: Partial<{ customer: string; subscription: string; periodEnd: number; audience: string }> = {}): Stripe.Event {
  return {
    id: `evt_${type}_1`,
    type,
    api_version: "2024-12-18.acacia",
    ...
    data: {
      object: {
        ...
        customer: over.customer ?? "cus_test_1",
        subscription: over.subscription ?? "sub_test_1",
        subscription_details: { metadata: { audience: over.audience ?? "smb" } },
        lines: {
          data: [
            {
...
describe("handleStripeEvent · checkout.session.completed", () => {
  test("wires subscription id + plan onto an SMB user (matched by customerId)", async () => {
    const ctx = makePrisma({
      users: [{ id: "u1", stripeCustomerId: "cus_test_1", stripeSubscriptionId: null }],
    });
  ...
  test("agency plan also updates Agency.plan tier", async () => {
    const ctx = makePrisma({
      agencies: [{ id: "a1", stripeCustomerId: "cus_agency_1", stripeSubscriptionId: null, plan: "SOLO" }],
    });
  ...
  test("ignores non-subscription mode sessions (one-time payment)", async () => {
    ...
    const out = await handleStripeEvent(checkoutCompleted({ mode: "payment" }), ctx.seam);
  ...
  test("skips when plan literal is unknown (logs no write)", async () => {
    ...
    const out = await handleStripeEvent(
      checkoutCompleted({ metadata: { userId: "u1", plan: "vip_supreme", audience: "smb" } }),
      ctx.seam,
    );
...
describe("handleStripeEvent · customer.subscription.updated", () => {
  test("syncs status + priceId + currentPeriodEnd onto matching user", async () => {
    const ctx = makePrisma({
      users: [{ id: "u1", stripeCustomerId: "cus_test_1", stripeSubscriptionId: "sub_test_1" }],
    });
    ...
    expect((ctx.userUpdates[0].data.currentPeriodEnd as Date).getTime()).toBe(1717000000 * 1000);
  });

  test("agency upgrade flips Agency.plan enum", async () => {
    const ctx = makePrisma({
      agencies: [{ id: "a1", stripeCustomerId: "cus_agency_1", stripeSubscriptionId: "sub_agency_1", plan: "SOLO" }],
    });
...
describe("handleStripeEvent · customer.subscription.deleted", () => {
  ...
  test("downgrades agency to SOLO sentinel", async () => {
    const ctx = makePrisma({
      agencies: [{ id: "a1", stripeCustomerId: "cus_agency_1", stripeSubscriptionId: "sub_agency_1", plan: "BOUTIQUE" }],
    });
    ...
    await handleStripeEvent(
      subscriptionEvent("customer.subscription.deleted", {
        ...
        customer: "cus_agency_1",
        status: "canceled",
        metadata: { agencyId: "a1", plan: "agency_boutique", audience: "agency" },
      }),
      ctx.seam,
    );
...
describe("handleStripeEvent · invoice.paid", () => {
  test("refreshes currentPeriodEnd from the line-item period.end", async () => {
    ...
    const out = await handleStripeEvent(invoiceEvent("invoice.paid", { periodEnd: 1717999999 }), ctx.seam);

    expect(out.kind).toBe("handled");
    expect(ctx.userUpdates[0].data).toMatchObject({ stripeStatus: "active" });
    expect((ctx.userUpdates[0].data.currentPeriodEnd as Date).getTime()).toBe(1717999999 * 1000);
  });

  test("ignores invoices with no subscription (one-off charges)", async () => {
    const ctx = makePrisma({});
    const out = await handleStripeEvent(invoiceEvent("invoice.paid", { subscription: "" }), ctx.seam);
    expect(out.kind).toBe("ignored");
  });
});
