...
| FT-2 Search everywhere     | modules/discovery/search-index-actions       | `(agency)/discover` flow (search mode) — cross-market signal search, charge-per-delivered, "delivered" research (SE · goal name) | live   |
| Agency ops                 | modules/{campaign,outreach,agency-settings}  | `(agency)/` — research (rename + pin) · campaigns · touchpoints · usage · team · setup · settings                                | live   |
