# Incidents archive · superseded + fully-encoded entries

Moved out of `incidents.md` per the archival policy in
`.claude/rules/incident-prevention.md`. Every entry here is either explicitly
SUPERSEDED or resolved with its prevention fully absorbed by a rule file.
Grep on demand (`grep -n "INC-29" incidents-archive.md`) — do not read wholesale.
The active file keeps a one-line stub per entry under `## Archived index`.
...
### INC-2026-05-21-40 · Function props across server→client boundary on `/lists/[id]` · INC-26 broader expression
...
Functions can't be serialized through the server→client boundary. Same incident class as **INC-26** (`t.rich()` render props) — just expressed via direct function props instead. INC-26's rule was specific to `t.rich`; the underlying class is broader: **ANY function prop crossing into a client component fails.**

**Fix applied (PR #83):**

1. **Pre-resolve per-row server-side**: added `openAriaLabel: string` to `InteractiveLeadRowData`. The page resolves it inside the existing `.map()` over leads, so each row carries its own plain string across the boundary.

   ```ts
   // page.tsx
   leads={data.leads.map<InteractiveLeadRowData>((lead) => ({
     // ... other fields ...
     openAriaLabel: t("table_open_aria", { business: lead.businessName }),
   }))}
   ```

2. **Resolve client-side via `useTranslations`**: the selectedCount varies client-side, so we can't pre-resolve. The client island calls `useTranslations(namespace)` directly:
...
3. **Mechanical detector candidate.** A pre-merge grep: any `'use client'` component whose props interface declares a field of type `(...args: ...) => ...` is a candidate defect. Type-system enforcement (e.g. a branded `ServerToClientSafe<T>` wrapper) would catch this at compile time — currently TypeScript happily allows function props to cross the boundary because the runtime check is in React, not the type system.

**Where encoded:**

- This entry (broaden INC-26 to all function props, not just `t.rich`)
- `modules/agency-portal/list-detail/components/LeadsTableInteractive.tsx` (`openAriaLabel` per-row + `useTranslations` client-side)
- `app/[locale]/(agency)/lists/[id]/page.tsx` (drop the two function-prop labels)
- **TODO follow-up**: update `.claude/rules/cache-components.md` Pattern 4 to explicitly forbid all function props across the boundary, not just `t.rich` render props. Currently the rule only mentions `t.rich`, which is why this regression slipped past code-reviewer agents.
