...
**Follow-up SHIPPED (2026-07-10, v0.19.23-pending) · the "Not available" cell state:** the fan-out/quote already excluded dead pairs (`permanentlyUnavailablePairs`), but the TABLE CELL still read the last FAILED job → "failed · retry" (clickable) forever, because the coverage matrix only knew failed-vs-not, not permanent-vs-retryable. Fix: a new terminal `TypeState` "unavailable" — `coverage-matrix.ts` computes the dead (business,family) pairs (same predicate as billing) and threads `unavailableJobFamilies` into `deriveTypeStates`; the cell renders a non-clickable "not available" (⊘, muted); `rowToGetGroups` (not_run||failed) already excludes it so dead leads drop out of the "Enrich N" count + the estimate. Precedence: a later SUCCESS clears it (data wins); a retryable `failed` sibling still wins in a group (actionable). Note: these specific leads carry the OLD `contacts_fetch_failed` (pre-INC-59 deploy) — they're dead via the 3-distinct-runs cap, NOT the new dns/parked reason; both paths now render "not available".
**Still deferred (named):** `Business.websiteStatus` column; per-reason cell copy ("Website gone" vs "Domain parked" vs "No source" — needs the reason surfaced through coverage); identity-mismatch layer (domain resold to a DIFFERENT real business — needs GBP identity anchors); registry signals `website_dead` / `domain_parked` (a premium agency prospecting signal — a business with reviews but a dead domain is the hottest "needs a website" lead).
...
### INC-2026-07-10-60 · enrichment "takes forever to start" + "shows running for minutes after done" = a silent dead accelerator + a close that only a cron could fire

**Symptom:** Viktor: (a) clicking Run sat on "Enriching" for ~1 min before any progress; a $0 all-fresh 8-lead run "enriched" for over a minute then closed in 161ms. (b) "Enriching · 44 of 45 · updating live" persisted for ~4 min after all real work ended, with Meta still reading "running" long after its Apify run timed out.
**Root cause (three, all mechanical, all found in the DB CronRun/EnrichmentRun rows):** (1) **The start accelerator was silently dead in prod.** `kickDispatch()` (fire-and-forget POST to the dispatch drain, meant to skip the _/2 cron wait) `return Promise.resolve()`'d with NO log when `NEXT_PUBLIC_APP_URL`/`CRON_SECRET` were unset, and swallowed every fetch error. DB proof: every post-fix run fanned out at an even-minute+~2s boundary (the cron), with NO `enrichment:dispatch` CronRun between the click and the tick — the kick never landed, so every run ate 0–120s (avg ~60s) of dead time. Same class as INC-58's "a fallback that fires ~always means the primary is broken", but here the fallback was INVISIBLE, so it read as "enrichment is just slow" for as long as the env was misconfigured. (2) **Run-close only ever happened on a dispatch tick.** `closeRunIfDone` was called ONLY from the _/2 cron's RUNNING-run loop; the Boxly worker callbacks (`/api/internal/enrich-job`) bumped the progress counter but NEVER attempted a close — so a run whose LAST job drained on the worker lane stayed RUNNING until the next cron noticed (up to 2 min). (3) **The retry-backoff tail had no UI signal.** Two `contacts_fetch_failed` jobs rode the `2^attempts`-minute ladder to attempt 3 (terminal at +4m08s); the run correctly stayed open (`closeRunIfDone` gates on zero QUEUED/RUNNING), but the progress bar showed a frozen "44 of 45" because `retrying` wasn't a counter. Meta showed "running" for the same window because the per-cell "enriching…" flag clears only at RUN-terminal (`emitEnrichFinished`), not when the FAILED AdMarketRun lands ~11s after the actor timeout.
**Fix applied:** (1) `kickDispatch` now WARNs (structured JSON) on missing-env, non-2xx (401/Deployment-Protection), and network error — a dead accelerator is now visible in Vercel logs instead of presenting as slowness. (2) `/api/internal/enrich-job` calls `closeRunIfDone(runId)` best-effort after a terminal outcome — CAS-guarded via the `finishedAt` claim so a racing tick still settles credits EXACTLY once; a cheap no-op when jobs remain open (e.g. the DOM dependents just enqueued). (3) New `retrying` Redis counter (businesses whose only outstanding work is a future-`nextAttemptAt` QUEUED job, minus any with a RUNNING sibling); `updateRunProgress` seeds it, the progress route exposes it (in the ETag), and EnrichingStep + LiveRunGate render "· N retrying". Progress counters are also seeded at ENQUEUE ({0 of N · PENDING}) so the first poll is coherent instead of a DB-fallback 0.
**Prevention (mechanical):** (a) a "best-effort" side-channel accelerator (self-fetch, webhook kick, cache warm) MUST log every skip/failure — a silent no-op that fires on the common path is indistinguishable from "the feature is just slow" and can rot for months; grep guard: a `catch {}` / bare `return` in a kick/dispatch/warm helper is a review flag. (b) when a terminal state transition can be driven by an ASYNC lane (worker callback, queue consumer) as well as a poller, the async lane must attempt the terminal/close action too — never assume the poller will notice "soon"; the CAS/idempotency guard that makes double-firing safe is the enabler. (c) a progress surface that can legitimately stall (retry backoff, rate-limit wait) needs a distinct signal for WHY it's paused, or it reads as frozen/broken.
**Deferred (M-effort, named):** per-cell Meta/SERP terminal state in the progress payload + a workbench "Meta: not available (collection failed)" chip so a cell flips at AdMarketRun-landing not run-terminal; a launched-attempt RUNNING AdMarketRun at chunk-launch + reconcile-flip so a dead enrich-cell callback can't hang a run to the 15-min ceiling; blocked-cell Meta cooldown (skip re-launch after N timeouts/24h).
**Confidence:** high (every lag reconstructed from real CronRun/EnrichmentRun/EnrichmentJob timestamps in prod DB — 5-agent forensics).
**Amendment (2026-07-10, same day) · "Selected (8)" phantom re-reported:** the v0.19.21 fix (clear-selection-on-enqueue + "Enrich selected N" label) wasn't enough — a STALE row selection still drove the toolbar's default run before the user ran anything. Owner's rule: "show for visible here [the toolbar], and show selected only if the user selects leads in the table." Correct fix = DECOUPLE the toolbar from the selection entirely: the toolbar "Enrich N" button ALWAYS targets the VISIBLE (filtered-enrichable) set and opens the sheet on `defaultScope:"visible"`; the SELECTED scope is reachable ONLY via the bulk-action bar, which appears only when the user checks rows (so it's inherently deliberate + shows a Clear affordance). The sheet now takes an explicit `defaultScope` from the opener (`resolveDefaultScope` pure helper, unit-tested) instead of inferring from `selectedIds.length>0` — which was the root of the phantom. Prevention: a default that infers "user intent" from PERSISTENT client state (a selection Set that survives soft-nav + runs) will mis-fire — make the OPENER declare intent explicitly; ephemeral selections must not seed a billed default.
**Where encoded:** this entry; kick-dispatch.ts (WARN paths); enrich-job/route.ts (close-on-callback); dispatch.ts updateRunProgress (retrying) + closeRunIfDone terminal seed; run-progress-counter.ts (retrying key, +test); enrich-actions.ts (enqueue seed); progress/route.ts; EnrichingStep.tsx + LiveRunGate.tsx; enrich-sheet-bus.ts (`resolveDefaultScope` + `defaultScope`, +test); EnrichMoreSheet.tsx + LeadsWorkbench.tsx (toolbar=visible, bulk=selected); [[run-forensics-fixes-shipped]].
**Confidence:** high (every lag reconstructed from real CronRun/EnrichmentRun/EnrichmentJob timestamps in prod DB — 5-agent forensics).
**Tags:** enrichment, dispatch, progress, run-close, kick-dispatch, silent-fallback, worker-callback, retry-backoff, prod-contact, observability, scope-default, stale-selection

### INC-2026-07-10-61 · a self-rescheduling poll with an un-caught `await` server-action died on ONE throw and left the mapping UI "Querying… 31s" forever

**Symptom:** the /discover PreviewStep showed "Querying Google Maps across 1 market… 31s" stuck for 10+ minutes on a coffee_shop|kelowna map. DB proof: the discovery (cmrf7cgjm) actually COMPLETED at 17:26:13 with 101 businesses (DISCOVERED_NEW) — the work was fine; the UI just never updated (a refresh showed the results). The frozen "31s" = the poll died ~31s in.
**Root cause:** PreviewStep's discovery-status poll re-schedules itself (`timer = setTimeout(poll, 2000)` at the end of each pass). It handled a RETURNED `status !== "ok"` but NOT a THROWN error: `const s = await getDiscoverySummary({discoveryId})` had no try/catch, so a single throw (transient network blip, a version-skew server-action reference after one of the day's deploys, a cold-start hiccup) rejected `poll()` BEFORE it scheduled the next timer → the loop was dead permanently. The discovery finished 5 min later but nothing was left polling to see it.
**Fix applied:** wrap the `getDiscoverySummary` call in try/catch; on a throw, reschedule (`setTimeout(poll, 2000)`) when not cancelled + requestId still matches, then return — so the loop SURVIVES the blip and lives to see READY. jobStatus is set before the terminal branch, so even a later sample-fetch throw still advances the UI.
**Prevention (mechanical):** a poll/loop that RESCHEDULES ITSELF from inside its own body (`setTimeout(self, …)`) must treat the whole body as fallible — every `await` needs a try/catch whose catch RE-ARMS the next tick, or one throw silently ends the loop forever. Grep guard for review: a `setTimeout(<sameFn>` inside a function whose body has a bare `await …Action(`/`await fetch(` with no enclosing try. This is the mirror of INC-60's worker-callback lesson (async lanes need their own guards) — here the guard is "never let the rescheduler's own body throw past the reschedule point." Bonus: this class gets MORE likely right after a deploy (server-action version skew), which is also when [[github-account-flagged]]-style blank/stuck states cluster — ties to the landing-page version-skew finding (no root error boundary + edge-cached HTML → ChunkLoadError; recommend Vercel Skew Protection).
**Also shipped alongside (UX, not a bug):** NEAR-DONE backoff cap (`requeueBackoff`) — a straggler in a ≥90%-complete run retries in ≤60s instead of the full 1→2→4-min ladder, so one dead-ish site (thebreadcompany.ca contacts_fetch_failed) can't hold a 54-of-55 run's "Enriching… 1 retrying" banner for 6+ min. The full ladder still applies mid-flight (rate-limited vendors unaffected); MAX_JOB_ATTEMPTS=3 still bounds total retries; billing untouched (only nextAttemptAt timing). The OTHER half of the same slowness is the broken kick-dispatch (a due retry waits up to 2 min for the \*/2 cron) — Viktor's Vercel env fix (NEXT_PUBLIC_APP_URL + CRON_SECRET).
**Where encoded:** this entry; PreviewStep.tsx (poll try/catch); dispatch.ts (`requeueBackoff` near-done cap + exported `backoffUntil`, +3 tests); [[run-forensics-fixes-shipped]].
**Confidence:** high (stuck state reproduced against the live DB — discovery done + 101 businesses while the UI showed 31s; poll-death mechanism is a direct read of the code).
**Tags:** discovery, preview, poll-loop, unhandled-throw, server-action, version-skew, retry-backoff, near-done-cap, prod-contact

### INC-2026-07-10-62 · a new Apify actor hung 280 s at 0 records because `Actor.openKeyValueStore(NAME)` jams the SDK storage queue under LIMITED_PERMISSIONS

**Symptom:** the new `mapsly-meta-graphql` actor (build/test cycle) timed out at the 280 s run limit producing ZERO dataset records + no RUN_SUMMARY, on EVERY run, regardless of code changes to the crawler/handler. stdout showed only Crawlee's "System info" then silence (buffered stdout is lost on the SIGKILL-at-timeout). Cost climbed smoothly like a full browser run, so *something* was executing.
**Root cause:** KV-store breadcrumbs (setValue "BC_LAST" — an immediate PUT that survives SIGKILL, unlike buffered pushData/stdout) pinpointed it: execution reached `init-done` but never the next module line, which was `const warmStore = await Actor.openKeyValueStore("mapsly-meta-warm")` — opening a NAMED, account-level KV store (for cross-run `datr` warming). Apify actor runs execute under `LIMITED_PERMISSIONS`; opening a named account-level store by name **hangs instead of rejecting** there (so the `.catch(()=>null)` never fires), AND — the load-bearing part — the pending storage call **jams the Apify SDK's internal storage queue**, so every later `Actor.setValue`/`Actor.pushData` (both my breadcrumbs AND the crawler's real scraped records) blocks behind it forever while the crawler keeps running + burning residential proxy. A `Promise.race` timeout on the open() did NOT help — the background call still holds the queue.
**Fix applied:** drop the named cross-run store entirely (`const warmStore = null;` + all uses guarded) — each run primes its own `datr`, so cross-run warming was a marginal trust optimization not worth a hard hang. Actor then ran cleanly (28 s, real data).
**Prevention (mechanical):** in an Apify actor, NEVER open a named account-level storage (`Actor.openKeyValueStore(name)` / `openDataset(name)` with a string arg) in the run's hot path — it can hang under LIMITED_PERMISSIONS and a hung storage op **stalls ALL subsequent storage writes** (silent data loss that looks like a scraper bug). Use the run's DEFAULT store (no-arg) only, or gate a named store behind a real feature need with a hard timeout AND accept it may never resolve. Diagnostic pattern that cracked this: when an Apify run hangs at 0 records with buffered-stdout loss, write an ever-overwritten `Actor.setValue("BC_LAST", {step})` at each phase — KV PUTs are immediate and survive SIGKILL, so BC_LAST names the exact last line reached. (The old `mapsly-meta-ad-library` actor opens the same named store at top — it only works because that store already exists + is reachable on its runs; treat any named-store open as a latent hang risk.)
**Where encoded:** this entry; `apify-actors/meta-graphql/src/main.js` (warmStore=null + BC_LAST diag pattern); [[meta-actor-robustness]]; report `docs/meta-graphql-actor-build-2026-07-10.html`.
**Confidence:** high (isolated via KV breadcrumbs to the exact line; removing the call fixed it deterministically across many runs).
**Tags:** apify, actor, kv-store, limited-permissions, storage-queue-stall, silent-data-loss, hang, diagnostics, meta-ads

### INC-2026-07-15-63 · GTM loaded at mount raced React 19's destructive streaming reveal → permanent white homepage on cold loads

**Symptom:** `www.mapsly.ai/` and `/for-agencies` rendered a **hard white page on first (cold-cache) load** for some users; a reload fixed it. Console: `Uncaught HierarchyRequestError: Failed to execute 'insertBefore' on 'Node': The new child element contains the parent. at $RV`. Server HTML was never at fault — `curl` returns 200 / 0 hops / 449 kB of fully server-rendered content every time. Long misdiagnosed as a redirect (none exists — see [[goal-state-lives-in-url]]-style stale premise) and then as a render-blocking Google Fonts stall (a real perf nit, NOT this).
**Root cause:** React 19 does not reveal streamed Suspense content immediately — `$RC` schedules `$RV` via `setTimeout($RV, $RT+300-now)`, i.e. **~300 ms after first paint**. `$RV` is **destructive and non-atomic**: it does `f.removeChild(c)` for every placeholder node FIRST, then `f.insertBefore(e.firstChild, c)`. `GtmContainer` loaded GTM from a `useEffect` on **mount** — i.e. at hydration, squarely inside that ~300 ms window. GTM then injects the Meta Pixel `<script>`, its `<noscript>` pixel, and the Smartlook loader as **direct `<body>` children**, and Smartlook's recorder begins instrumenting/re-parenting the DOM. `$RV` then runs against a tree it no longer recognises, throws mid-reveal **after** the removals, and the content is *destroyed, not merely hidden* — `div[hidden]` measured 17 bytes (`<!--$--><!--/$-->`), `document.body.innerText.length === 0`, and it never recovers. Cold loads lose the race (subresources spread the timeline); warm reloads win it — hence "some users, first load only".
**Fix applied:** `modules/analytics/GtmContainer.tsx` — load GTM on the `load` event (plus one `requestAnimationFrame`) instead of on mount, so every GTM-managed tag lands strictly after the reveal has settled. `readyState === "complete"` short-circuit + listener cleanup for the soft-nav case.
**Prevention (mechanical):** **No third-party tag may touch the DOM before `load`.** Any analytics/recorder/pixel loader in this repo must be gated on `load` (or later) — never on mount, never `beforeInteractive`. Grep for the anti-pattern: `grep -rn "useEffect" modules/analytics/` — every loader there must reference `readyState`/`"load"`. Corollary for review: a tag that appends to `<body>` is not "outside React" — under streaming SSR, `<body>` is React's live workspace for ~300 ms after first paint. Diagnostic recipe for any future "blank page but curl is fine": read `document.body.innerText.length` + `document.querySelector('div[hidden]').innerHTML.length` — a ~17-byte hidden holder with 0 body text is this exact `$RV` abort, and it is *always* a client-side DOM-mutation race, never a server bug.
**Where encoded:** this entry; `modules/analytics/GtmContainer.tsx` (inline comment cites this INC); report `docs/agency-landing-audit-2026-07-15.html`.
**Confidence:** high on the mechanism (crash reproduced live; destroyed-not-hidden content measured directly; cold-load-white vs warm-reload-fine reproduced back-to-back). Medium on *which* GTM tag corrupts the tree — Smartlook is the prime suspect as the only DOM-rewriting tag, but the fix removes the whole window regardless, so isolating it is not load-bearing.
**Tags:** react-19, streaming-ssr, suspense, hydration, gtm, smartlook, meta-pixel, third-party, white-page, race-condition, marketing-v2
**AMENDMENT 2026-07-15 (SUPERSEDED — this diagnosis was WRONG):** The GTM-race story was a plausible mis-attribution. The `$RV` crash kept happening on a build where GTM loads only at `load` (so it cannot be inside the ~300 ms reveal window). The real cause is a duplicate-DOM-id collision from PPR's two-render output — see **INC-2026-07-15-64**, which is the true root cause. The GTM `load`-gating change is still correct hygiene (a tag mutating `<body>` during streaming SSR is genuinely unsafe) and was kept, but it was NOT the fix. Lesson for the process: I shipped this "fix" to production confidently before proving it stopped the crash — verify the fix reproduces-then-stops the symptom BEFORE shipping, not after.

### INC-2026-07-15-64 · PPR two-render segment-id collision (`$RS`/`$RV` HierarchyRequestError) → permanent white page on every marketing route

**Symptom:** Same console error as INC-63 — `Uncaught HierarchyRequestError: Failed to execute 'insertBefore' on 'Node': The new child element contains the parent. at $RV` — on `/`, `/for-agencies`, `/for-businesses`, all `/compare/*`, every locale. Intermittent (a race) but the malformed HTML shipped on 100% of requests. `curl` always returns a healthy 200 / ~449 kB; the destruction is 100% client-side. INC-63's GTM fix did NOT stop it.
**Root cause (proven — jsdom-reproduced against the real production HTML, and reproduced/fixed on a local `next start`):** Under `next.config.ts` `cacheComponents: true` (PPR), each marketing document is **TWO React Fizz renders concatenated into one HTML file**: a build-time prerendered shell (scripts have NO nonce, bytes 0–~131 k) + a per-request dynamic resume (scripts nonced — the nonce is minted per-request in `middleware.ts`, everything after ~131 k). The dynamic resume exists because `V2Header` in `app/[locale]/(marketing-v2)/layout.tsx` did `await auth()` (a cookie read) inside a `<Suspense>` boundary → the boundary **postpones** at build. React's Fizz numbers streamed segments from a single `nextSegmentId` counter, and in the vendored React that Next 16.2.6 ships the resume restarts that counter from a **stale snapshot**, re-emitting `id="S:1"` / `id="S:2"` — ids the shell already used. Proof in the served bytes: `grep -o 'id="S:[0-9]*"' | sort | uniq -d` → `2× id="S:1"`, `2× id="S:2"` (S:3–S:9 unique; the 9 `$RS` segments are the outlined `FbLogo` `<path>` chunks). `document.getElementById("S:1")` returns the FIRST match — the shell's `<div hidden id="S:1">` that wraps the whole page — so the resume's `$RS("S:1","P:1")` moves the entire page subtree into the header logo `<svg>`, then `$RC("B:0","S:0")` → `$RV` runs `insertBefore` of an ancestor into its own descendant → the exact error, with `$RV`'s pre-insert `removeChild` explaining the destroyed DOM (17-byte `div[hidden]`, 0 body text). Race: when the rAF/`setTimeout`-deferred `$RV` happens to run before `$RS`, it harmlessly cleans up and the page renders — hence intermittent.
**Fix applied (v0.19.37):** Remove the `await auth()` + `getPortalDestination` from `V2Header` (and the now-dead portal props from `V2NavLinks`). The header is now fully static → all 24 marketing routes are a **single static render** → one id counter → collision structurally impossible. The signed-in "Open your workspace" CTA was dropped (it was the only per-request dynamic read on the highest-traffic routes); `/signin` already redirects an authenticated visitor to their portal (`app/[locale]/signin/page.tsx`), so the static "Sign in" CTA still lands them there in one click — only the label differs. If restored later, it MUST be a client island that reads session AFTER hydration, never a server read in the header.
**Prevention (mechanical):** **No server component in a marketing/public layout may read cookies/session/`headers()` inside (or above) a `<Suspense>` boundary** — that makes the boundary postpone and splits the document into shell+resume, which is where the id collision lives. The load-bearing check is on the SERVED document, not the built shell: run `next start`, `curl` each public route, and assert `grep -o 'id="S:[0-9]*"' | sort | uniq -d` is EMPTY and `$RS(` count is 0. Add to the review checklist for any `(marketing*)` route: "does any server component in the render path call `auth()` / `cookies()` / `headers()`? If yes, move it to a client island." Do NOT rely on grepping the build `.html` — the `postponed` marker can persist in `.meta` while the *served* resume is empty; only the served doc proves it.
**Where encoded:** this entry; `app/[locale]/(marketing-v2)/layout.tsx` + `components/marketing/for-businesses/V2NavLinks.tsx` (inline comments cite this INC); supersedes the INC-63 diagnosis.
**Confidence:** high. Mechanism reproduced end-to-end in jsdom against the real prod bytes; duplicate ids confirmed on the live deploy; fix reproduced on a local `next start` — 6/6 marketing routes served with `$RS=0`, zero duplicate ids, real `<h1>`, BEFORE deploying. The precise React-internal "why" (early `getPostponedState` snapshot in the vendored canary) is source-and-arithmetic strong, not live-instrumented, but is not load-bearing for the fix.
**Tags:** react-19, next-16, cacheComponents, ppr, postpone, resume, segment-id, duplicate-id, suspense, auth, white-page, race-condition, marketing-v2
